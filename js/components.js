/* ==========================================================================
   ...
   Reactive: observedAttributes + attributeChangedCallback re-render
   when Alpine changes :page / :color / :message.
   ========================================================================== */

var WHATSAPP_NUMBER = '5491131845820';
var WHATSAPP_PHONE = '+54 9 11 3184-5820';

var WHATSAPP_SVG = '<path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>';

/* ---------- Footer data per page (hash-based links) ---------------------- */
var FOOTER_DATA = {
    home: {
        ...
        links: [
            ...
            { text: 'Análisis Yeso', href: '#/analisis' },
            { text: 'Análisis Dolomita', href: '#/analisis-dolomita' },
...
/* =========================================================================
   <okomix-nav :page="currentPage" :color="navColor">
   ========================================================================= */
class OkomixNav extends HTMLElement {
    static get observedAttributes() { return ['page', 'color']; }
    connectedCallback() { this._render(); }
    attributeChangedCallback() { if (this.isConnected) this._render(); }

    _render() {
        var color = this.getAttribute('color') || 'oro';

        /* Color-dependent tokens */
        var hoverCls   = 'hover:text-' + color + '-400';
        var ctaClasses = color === 'oro'
            ? 'bg-oro-500 hover:bg-oro-600 text-mineral-900'
            : 'bg-campo-500 hover:bg-campo-600 text-white';
        var mobileCtaClasses = color === 'oro'
        ...
        this.innerHTML = '<nav class="fixed top-0 inset-x-0 z-50 bg-mineral-900/95 backdrop-blur-md border-b border-mineral-800" x-data="{ mobileMenuOpen: false }">'
            ...
            /* Logo */
            + '<a href="#" data-umami-event="nav-logo" class="flex items-center gap-3">'
            ...
            +   '<a href="#productos" data-umami-event="nav-click" data-umami-event-item="productos" class="text-mineral-300 ' + hoverCls + ' transition-colors font-medium">Productos</a>'
            +   '<a href="#beneficios" data-umami-event="nav-click" data-umami-event-item="beneficios" class="text-mineral-300 ' + hoverCls + ' transition-colors font-medium">Beneficios</a>'
            +   '<a href="#exportacion" data-umami-event="nav-click" data-umami-event-item="exportacion" class="text-mineral-300 ' + hoverCls + ' transition-colors font-medium">Exportación</a>'
            +   '<a href="#contacto" data-umami-event="nav-click" data-umami-event-item="contacto" class="' + ctaClasses + ' px-6 py-2.5 rounded-lg font-semibold transition-all hover:scale-105">Contactar</a>'
            + '</div>'
            /* Mobile hamburger */
            ...
            +   '<a href="#productos" @click="mobileMenuOpen = false" data-umami-event="nav-click" data-umami-event-item="productos" class="block py-3 px-4 text-mineral-200 hover:bg-mineral-700 rounded-lg">Productos</a>'
            +   '<a href="#beneficios" @click="mobileMenuOpen = false" data-umami-event="nav-click" data-umami-event-item="beneficios" class="block py-3 px-4 text-mineral-200 hover:bg-mineral-700 rounded-lg">Beneficios</a>'
            +   '<a href="#exportacion" @click="mobileMenuOpen = false" data-umami-event="nav-click" data-umami-event-item="exportacion" class="block py-3 px-4 text-mineral-200 hover:bg-mineral-700 rounded-lg">Exportación</a>'
            +   '<a href="#contacto" @click="mobileMenuOpen = false" data-umami-event="nav-click" data-umami-event-item="contacto" class="block py-3 px-4 ' + mobileCtaClasses + ' rounded-lg text-center font-semibold">Contactar</a>'
            + '</div></div>'
            + '</nav>';

        // Re-initialize Alpine tree so x-data/x-show inside nav works
        if (window.Alpine) Alpine.initTree(this);
    }
}

/* =========================================================================
   <okomix-footer :page="currentPage" :color="navColor">
   ========================================================================= */
class OkomixFooter extends HTMLElement {
    ...
    _render() {
        ...
        var color = this.getAttribute('color') || 'oro';
        var data  = FOOTER_DATA[page] || FOOTER_DATA.home;
